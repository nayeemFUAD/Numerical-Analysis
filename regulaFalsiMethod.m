%Find Root of an Equation Using Regula Falsi or False Position Method
%TITAS P-74 EX-19
clear all
clc
f = @(x) (x.^3 - 2*x - 5);
...
elseif(f(lRange) * f(uRange) == 0)
    ...
    count = 0; i = 1; temp = 0;
    while(count == 0)
        x = (lRange*f(uRange) - uRange*f(lRange)) / (f(uRange) - f(lRange));
...
title("Finding Root Using Regula Falsi Method")
